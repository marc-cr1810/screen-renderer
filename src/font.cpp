...
// 5x7 font bitmap data (each character is 5 bytes, representing 7 rows with 5 bits each)
// Format: Each bit represents a pixel (1 = on, 0 = off)
// Characters 32-126 (space through ~)
...
auto font_t::get_character(char c) const -> bitmap_t
{
  const unsigned char *char_data = get_char_data(c);
  bitmap_t char_bitmap(CHAR_WIDTH, CHAR_HEIGHT);
  ...
  for (size_t row = 0; row < CHAR_HEIGHT; ++row)
  {
    ...
    {
      // Check if bit is set (reading from left to right)
      bool pixel_on = (row_data & (1 << (4 - col))) != 0;
      char_bitmap.set_pixel(col, row, pixel_on);
    }
  }
...
auto font_t::get_char_width() const -> size_t
{
  return CHAR_WIDTH;
}

auto font_t::get_char_height() const -> size_t
{
  return CHAR_HEIGHT;
}

auto font_t::get_char_data(char c) const -> const unsigned char *
{
  // Map character to index in font data (32-126)
  int index = static_cast<int>(c) - 32;

  // If character is out of range, return space character
  if (index < 0 || index >= 95)
  {
    index = 0; // Space
  }
